...
#### `/history`
显示当前对话历史，包含消息统计和 Token 使用状态。

```
> /history

对话历史:
消息数: 4 | Token 估算: 245 | 状态: safe
...
**状态说明：**
- `safe` - Token 使用正常 (< 70%)
- `warning` - Token 使用较高 (70-90%)
- `critical` - Token 使用临界 (> 90%)，建议 `/clear` 清除历史
...
```bash
> 我叫张三
📊 Final Answer: 你好张三！很高兴认识你！

> 我叫什么名字？
📊 Final Answer: 你刚才告诉我你叫**张三**！
...
└─────────────────────────────────────────────────────────────┘
                              ...
                              ▼
┌─────────────────────────────────────────────────────────────┐
│              BaseLLMService (对话管理层)                      │
│  ┌─────────────────────────────────────────────────────────┐│
│  │  职责:                                                 ││
│  │  • 跨对话上下文管理 (SessionChat)                        ││
│  │  • 意图识别 (关键词 + LLM 混合策略)                      ││
│  │  • 智能路由 (直接对话 ↔ 工具执行)                        ││
│  │  • Token 估算与历史压缩                                  ││
│  └─────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────┘
              ...
              ▼                               ▼
┌─────────────────────────┐      ┌──────────────────────────────┐
│    直接对话 (无工具)      │      │   工具执行 (ReActAgent)       │
│   SessionChat.chat()    │      │  • executeTask()              │
│                         │      │  • executeTaskStream()        │
│  快速响应日常对话         │      │  状态less任务执行器           │
└─────────────────────────┘      └──────────────────────────────┘
```
...
### v2.1.0
- ✨ 架构升级：分离对话层与任务执行层
- ✨ 新增 BaseLLMService 管理跨对话上下文
- ✨ 智能意图识别（关键词 + LLM 混合策略）
- ✨ `/history` 命令增强 Token 统计显示
- ✨ 上下文保持：自动记忆跨工具调用的对话
- ✨ 新增 `executeTask()` / `executeTaskStream()` 方法
